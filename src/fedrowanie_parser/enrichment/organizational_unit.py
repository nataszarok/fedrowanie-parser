...
def looks_like_unit(s):
    ...
    if (
        not s
        or len(s) > 180
        or ORGANIZATIONAL_CONTACT_RE.search(s)
        or ORGANIZATIONAL_MONEY_RE.search(s)
    ):
        return False
    if re.search(
        r"(?i)\b(?:lp\.?|l\.p\.?|wynagrodzeni\w*|forma zatrudn\w*|"
        r"nazwisko|imi[ęe]|do kiedy zatrudniony|kwota)\b",
        s,
    ):
        return False
    if re.fullmatch(
        r"(?i)(?:lekarz\s+)?(?:kierownik|asystent|zast[ęe]pca|z-ca).*oddzia[łl]u", s
    ):
        return False
...
def unit_from_raw_row(raw):
    ...
    if len(cells) == 4:
        ...
        if (
            ...
            and re.fullmatch(
                r"(?i)(?:umowa zlecenia|umowa o prac[ęe]|kontrakt|cywilnoprawna)",
                fourth,
            )
        ):
            return second
...
def section_unit_for_row(doc, raw_row):
    ...
    for line in reversed(lines[-80:]):
        ...
        if len(s.split()) > 16 and not s.endswith(":"):
            continue
        if re.search(
            r"(?i)\b(?:tutejszy|podmiot|informuje|świadczenia|zatrudnion|"
            r"udziela|oddziały:)\b",
            s,
        ):
            continue
        return s
