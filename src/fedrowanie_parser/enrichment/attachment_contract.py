"""Infer contract types from attachment-level document context."""
...
def split_attachments(case_text: str) -> list[AttachmentSection]:
    ...
    text = case_text or ""
    ms = list(ATTACHMENT_CONTRACT_ATTACHMENT_RE.finditer(text))
    out = []
...
def _labels(text: str) -> list[tuple[str, str, int]]:
    hits = []
    for label, pat in ATTACHMENT_CONTRACT_PATTERNS:
...
def _numbered_ad_zones(att: AttachmentSection) -> list[ContractZone]:
    ...
    for m in re.finditer(
        r"(?is)(?<!\d)([1-9])\)\s*(.{1,700}?)(?=(?:\s+[1-9]\)|#\s*Ad\.|\Z))", intro
    ):
        n = int(m.group(1))
...
def _meaningful_name(name) -> Optional[str]:
    ...
    if not s:
        return None
    if re.fullmatch(r"(?i)Lekarz(?:\s+(?:nr\s*)?\d+)?", s):
        return None
    return s
