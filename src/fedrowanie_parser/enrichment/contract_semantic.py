...
def infer_contract_from_record_and_section(doc: str, raw_row: str):
    ...
    m = list(
        re.finditer(
            r"(?i)forma\s+zatrudnienia\s*:\s*(kontrakt\w*|umow\w*\s+o\s+prac[ęe]|umow\w*\s+zlecen\w*)",
            tail,
        )
    )
    if m:
        ...
        if "kontr" in v:
            return ("kontrakt/cywilnoprawna", "Forma zatrudnienia: kontrakt")
        if "prac" in v:
            return ("umowa o pracę", "Forma zatrudnienia: umowa o pracę")
        if "zlecen" in v:
            return ("umowa zlecenia", "Forma zatrudnienia: umowa zlecenia")

    # Strong table/section labels immediately before a table/list.
    if re.search(
        r"(?is)(?:^|\|)\s*umow\w*\s+o\s+prac[ęe]\s*(?:\||$).{0,500}(?:\blp\b|pracownik|lekarz)",
        tail,
    ):
        return ("umowa o pracę", "nagłówek sekcji/tabeli: umowa o pracę")
    if re.search(
        r"(?is)wynagrodzenia\s+(?:lekarzy\s+)?zleceniobiorc\w*.{0,120}umow\w*\s+zlecen",
        tail,
    ):
        return ("umowa zlecenia", "sekcja zleceniobiorców na umowie zlecenia")
    if re.search(
        r"(?is)wynagrodzenia\s+lekarzy\s+zatrudnionych.{0,100}(?:stosunku\s+pracy|umow\w*\s+o\s+prac)",
        tail,
    ):
    ...
    if re.search(
        r"(?is)przyjmuj\w+\s+zam[oó]wieni\w*.{0,250}umow\w*.{0,250}"
        r"udzielani\w+\s+[śs]wiadcze[ńn]\s+zdrowotn\w*.{0,250}"
        r"art\.?\s*26.{0,80}art\.?\s*27",
        tail,
    ):
        return (
            "kontrakt/cywilnoprawna",
            "przyjmujący zamówienie; świadczenia zdrowotne art. 26/27",
        )

    # A whole following list explicitly described as civil-law medical services.
    if re.search(
        r"(?is)ma\s+zawarte\s+umow\w+\s+cywilno\s*[-–—]?\s*prawn\w+.{0,500}"
        r"wyp[łl]aci[łl].{0,250}wynagrodzeni\w+\s+nast[ęe]puj[ąa]cym\s+lekarz",
        tail,
    ):
        return (
            "kontrakt/cywilnoprawna",
            "lista lekarzy objętych umowami cywilnoprawnymi",
        )

    # Competition-based medical-service agreements followed by a list of businesses.
    if re.search(
        r"(?is)[śs]wiadczeni\w+\s+zdrowotn\w+.{0,180}na\s+podstawie\s+um[oó]w"
        r".{0,300}post[ęe]powani\w+\s+konkursow\w+.{0,350}"
        r"list[ęa]\s+podmiot\w+",
        tail,
    ):
        return (
            "kontrakt/cywilnoprawna",
            "świadczenia zdrowotne po postępowaniu konkursowym",
        )
