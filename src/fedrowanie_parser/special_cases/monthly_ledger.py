    ...
    for items in groups.values():
        ...
        out.append(
            AnnualEntity(
                ...
                entity_kind="person_like"
                if any(x.entity_kind == "person_like" for x in items)
                else items[0].entity_kind,
            )
        )
...
def read_case_text(db_path: str, institution_pk: int) -> tuple[int, str]:
    ...
    parts = [
        ...
        for r in con.execute(
            "SELECT text FROM case_pages WHERE case_pk=? ORDER BY rowid", (case_pk,)
        )
    ]
    con.close()
    return case_pk, "\n".join(parts)
...
def _safe_same_person(a: AnnualEntity, b: AnnualEntity) -> bool:
    ...
    if len(ta) >= 2 and len(tb) >= 2:
        ...
        if (
            sa == sb
            and (ga.startswith(gb) or gb.startswith(ga))
            and min(len(ga), len(gb)) >= 5
        ):
            return True
