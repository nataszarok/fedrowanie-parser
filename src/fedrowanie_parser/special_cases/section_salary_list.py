...
def _looks_like_amount_tail(s):
    return bool(
        re.search(r"\d[\d\s\[\]]*[,.]\d{2}|\d[\d\s\[\]]{2,}\s*z[łl]?\b", s, re.I)
    )
...
def _parse_amount(s):
    ...
    ms = list(
        re.finditer(r"(\d[\d\s]*[,.]\d{2}|\d[\d\s]{2,})\s*(?:z[łl]|PLN)?", x, re.I)
    )
    if not ms:
        return None
    t = ms[-1].group(1)
...
def _is_heading_line(s):
    ...
    if (
        len(s) <= 60
        and not re.search(r"(?i)\blekarz\b", s)
        and not re.search(r"\d[\d\s]*[,.]\d{2}", s)
    ):
        return True
    return False
