...
def parse_ocr_contract_amount_list(
    ...
    for line in page.splitlines():
        ...
        is_civil = bool(
            re.search(
                "(?i)(?:cywilno|qwilno|cwilno|ewiino|wiino).?prawna|umowa.{0,20}prawna",
                raw,
            )
        )
        ...
        toks = re.findall(
            "(?<!\\d)(\\d{1,7}(?:[ .]\\d{3})*(?:,\\d{2})|\\d{1,3}(?:\\.\\d{3})+(?:,\\d{2}))(?!\\d)",
            amount_source,
        )
        if not toks:
            continue
...
def parse_ocr_broken_numbered_salary_table(
    ...
    for raw in prefix.splitlines():
        ...
        m = re.fullmatch(
            r"(\d{1,3}(?:[ .]\d{3})+|\d{4,7})(?:,\d{1,2})?\s*[A-Za-z|_]?", line
        )
        if not m:
            continue
...
def parse_forma_name_amount_ocr(case_pk, institution_pk, placowka, page_no, page):
    ...
    if not (
        re.search(r"(?m)^FORMA\s*$", page)
        or re.search(r"(?m)^FORMA\s+[A-ZĄĆĘŁŃÓŚŹŻ]", page)
    ):
        return []
    ...
    for raw in block.splitlines():
        ...
        for tok, val in cells:
            ...
            if not label or not re.search(r"[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż]", label):
                continue
            if is_metadata_context(label) or val <= 0 or val > 5_000_000:
                continue
            if re.search(r"(?i)kierownik\s+dzia[łl]u|koniec\s+strony", label):
                continue
            out.append(
                SalaryRow(
                    case_pk,
...
def parse_lekarz_inline_anon_list(case_pk, institution_pk, placowka, page_no, page):
    ...
    for raw in lines:
        m = re.fullmatch(
            "(?i)Lekarz\\s+(\\d{1,4})\\s*(?:\\||[-–—])?\\s*(\\d[\\d .]*,\\d{2})", raw
        )
        if m:
            val = parse_money(m.group(2))
            if val and val > 0:
