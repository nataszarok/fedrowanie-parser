...
def parse_plain_lines(case_pk: int, institution_pk: Optional[int], placowka: str, page_no: int, page: str) -> list[SalaryRow]:
    ...
    inline = re.compile('^(?:\\|\\s*)?(?P<idx>\\d{1,4})[.)]?\\s+(?P<label>.*?)(?:\\s*[—–-]\\s*|\\s{2,})(?P<amount>(?:\\d{1,3}(?:[ .]\\d{3})+|\\d{4,12})(?:[,.]\\d{1,2})?\\s*(?:zł|PLN)?)\\s*\\|?$', re.I)
    idx_amount = re.compile('^(?P<idx>\\d{1,4})[.)]*\\s+(?P<amount>\\d{1,3}(?:[ .]\\d{3})+(?:[,.]\\d{1,2})?|\\d{4,7}(?:[,.]\\d{1,2})?)\\s*(?:zł)?$', re.I)

    def near_summary_marker(pos: int, radius: int=10) -> bool:
        lo = max(0, pos - radius)
        hi = min(len(lines), pos + radius + 1)
        ctx = ' '.join(lines[lo:hi])
        return bool(re.search('(?i)\\b(?:razem|ogółem|ogolem|suma|łącznie|lacznie)\\s*:?', ctx))
    consumed = set()
    for n, line in enumerate(lines):
        if not line or '|' in line:
            continue
        if is_metadata_or_date(line) or mentions_other_year(line) or is_summary_row(line):
            continue
        m = inline.match(line)
        if m:
            label_probe = norm_space(m.group('label'))
            if (not label_probe or re.fullmatch('\\d*', label_probe)) and near_summary_marker(n):
                continue
    ...
    for n in range(len(lines) - 2):
        ...
        if any((is_summary_row(x) or is_metadata_or_date(x) for x in window)):
            continue
        if _looks_like_document_metadata_window(lines[n:n + 6]):
            continue
        amount_pos = None
        val = None
        for j, x in enumerate(window, 1):
            ms = money_cells(x)
            if len(ms) == 1 and re.fullmatch('\\d{1,3}(?:[ .]\\d{3})*(?:[,.]\\d{1,2})?\\s*(?:zł)?', x, re.I):
                amount_pos = n + j
                val = ms[0][1]
                break
        if val is None:
...
def parse_section_state_rows(case_pk: int, institution_pk: Optional[int], placowka: str, doc: str) -> list[SalaryRow]:
    ...
    sectionish = re.compile('(?i)(?:umow[ayę]\\s+o\\s+prac|umow[ayę]\\s+zlecen|zlecenie|kontrakt|cywilno[- ]?prawn)')
    ...
    for raw in (doc or '').splitlines():
        ...
        if not in_salary_section or not current_contract:
            continue
        if is_separator_row(cells) or is_summary_row(line) or is_metadata_or_date(line) or mentions_other_year(line):
            continue
        if len(cells) == 2:
        ...
        if len(cells) == 3:
            c0, c1, c2 = cells
            vals = money_cells(c2)
            headerish = bool(re.fullmatch('(?i)nazwisko', c0)) or bool(re.fullmatch('(?i)imi[ęe]', c1)) or bool(re.search('(?i)^wynagrod', c2))
            text_identity = bool(c0 and c1 and (not money_cells(c0)) and (not money_cells(c1)))
...
def _looks_like_document_metadata_window(lines: list[str]) -> bool:
    """Return whether nearby vertical values are document-processing metadata."""
    normalized = [norm_space(line) for line in lines if norm_space(line)]
    ...
    return (
        "documentemail" in joined
        or "x-none" in joined
        or (
            any(marker in joined for marker in technical_markers)
            and boolean_tokens >= 2
        )
    )
