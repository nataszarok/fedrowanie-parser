...
def parse_indexed_three_col_continuation(
    case_pk: int,
    ...
    for raw in page.splitlines():
        if not raw.strip().startswith("|") or re.fullmatch(
            "\\s*\\|?\\s*:?-{3,}:?\\s*(?:\\|\\s*:?-{3,}:?\\s*)+\\|?\\s*", raw
        ):
            continue
        ...
        if len(vals) != 1 or vals[0] <= 0:
            continue
        candidates.append(
            (int(re.sub("\\D", "", idx_txt)), mid, vals[0], norm_space(raw))
        )
    if len(candidates) < 3:
        return []
    rows = []
...
def parse_indexed_single_salary_markdown(
...
def parse_indexed_total_gross_continuation(
    case_pk, institution_pk, placowka, page_no, page
):
...
def parse_salary_bracket_index_table(
...
def parse_parallel_index_amount_columns(
    case_pk, institution_pk, placowka, page_no, page
):
    ...
    for raw in page.splitlines():
        ...
        c1 = norm_space(cc[1])
        c3 = norm_space(cc[3])
        money_only = re.compile(
            "^\\s*(?:\\d{1,3}(?:[ .]\\d{3})+|\\d{4,7})(?:,\\d{1,2})?\\s*(?:zł)?\\s*$",
            re.I,
        )
