...
def parse_numbered_named_inline_salary(
    case_pk, institution_pk, placowka, page_no, page
):
    ...
    for line in page.splitlines():
        ...
        if re.search("\\d+-\\d{3},\\d{2}\\b", raw):
            continue
        m = re.match(
            "^(\\d{1,4})[.)]\\s*(.+?)(?:\\s*[—–-]\\s*|\\s+)(\\d[\\d .]*,\\d{2})\\s*(?:(?:zł|zl|z)(?:\\s*brutto)?)?[,.]?$",
            raw,
            re.I,
        )
        if not m:
...
def parse_lekarz_inline_salary(case_pk, institution_pk, placowka, page_no, page):
    ...
    for line in page.splitlines():
        raw = norm_space(line)
        m = re.match(
            "(?i)^lekarz\\s+(\\d{1,4})\\s+(\\d{1,3}(?:[ .]\\d{3})*(?:,\\d{1,2}))\\s+(.+)$",
            raw,
        )
        if not m:
            continue
        ...
        out.append(
            SalaryRow(
                ...
                placowka,
                f"Lekarz {m.group(1)}",
                norm_space(m.group(3)),
                "",
                None,
                val,
                page_no,
                "lekarz-inline-specialty",
                "wysoka",
                raw,
            )
        )
...
def parse_embedded_numbered_salary_list(
    case_pk, institution_pk, placowka, page_no, page, full_doc=""
):
    ...
    pat = re.compile(
        "(?<!\\d)(\\d{1,3})[.)]\\s+(.{1,180}?)\\s*[-–—]\\s*(\\d{1,3}(?:[ .]\\d{3})+|\\d{1,7})(?:,\\d{1,2})\\s*(?:zł|zl|zlt|zhl|zt)\\b",
        re.I,
    )
    med = re.compile(
        "(?i)lekarz|chirurg|psychiatr|stomatolog|okulist|radiolog|anestezj|ordynator|asystent|rehabilitant|urolog|neurolog|kardiolog|internist|laryngolog|gastrolog|epidemiolog|ortoped|ginekolog|pediatr|endokrynolog|diabetolog|dyrektor szpitala|dyżury lekarskie|dyzury lekarskie"
    )
    out = []
    for m in pat.finditer(text):
        ...
        out.append(
            SalaryRow(
                ...
                val,
                page_no,
                "embedded-numbered-salary-list",
                "wysoka",
                norm_space(m.group(0)),
            )
        )
    explicit = bool(
        re.search("(?i)poniżej przedstawiam listę|ponizej przedstawiam liste", page)
    )
...
def parse_specialty_amount_lines(case_pk, institution_pk, placowka, page_no, page):
    ...
    annual_ctx = bool(
        ...
        or re.search(
            r"(?is)wynagrodzenia.{0,220}(?:podziale|stanowisk).{0,220}2025", page
        )
        or re.search(
            r"(?is)2025.{0,220}wynagrodzenia.{0,220}(?:stanowisk|podziale)", page
        )
    )
    if not annual_ctx:
        return []
    ...
    scope = page
    intro = re.search(
        r"(?is)wynagrodzenia.{0,220}kształtowały\s+się\s+następująco\s*:", page
    )
    if intro:
        scope = page[intro.end() :]
        scope = re.split(
            r"(?is)\bJednocześnie\s+informuję\b|\bZ\s+poważaniem\b|\bKlauzula\s+informacyjna\b",
            scope,
            maxsplit=1,
        )[0]
    out = []
    med = re.compile(
        r"(?i)ordynator|chirurg|psychiatr|stomatolog|okulist|lekarz|anestezj|rehabilitant|radiolog|urolog|neurolog|kardiolog|pulm|laryng|asystent|kierownik|dyrektor"
    )
...
def parse_explicit_annual_prose_salary(
    case_pk, institution_pk, placowka, page_no, page
):
    """Parse explicit annual prose salary layouts into salary-row candidates."""
    has_numbered_context = bool(
        re.search(r"(?is)zestawienie.{0,100}wynagrodze[nń].{0,100}2025", page)
    )
    has_single_explicit = bool(
        re.search(
    ...
    if has_numbered_context:
        ...
        for k, m in enumerate(marks):
            ...
            if len(chunk) > 500:
                continue
            wm = re.search(
                r"(?i)wynagrodzenie\s+brutto\s*:\s*(\d{1,3}(?:[ .]\d{3})+|\d{4,7})(?:,\d{1,2})",
                chunk,
            )
            if wm:
                token = re.search(
                    r"(\d{1,3}(?:[ .]\d{3})+|\d{4,7})(?:,\d{1,2})", wm.group(0)
                ).group(0)
                label = chunk[: wm.start()].strip(" ,;-—")
            else:
                am = list(
                    re.finditer(
                        r"(?<!\d)(\d{1,3}(?:[ .]\d{3})+|\d{4,7})(?:,\d{1,2})(?:\s*zł)?",
                        chunk,
                        re.I,
                    )
                )
                if not am:
                    continue
                token = am[-1].group(0)
                label = chunk[: am[-1].start()].strip(" ,;-—")
            if not re.search(r"[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż]", label):
                continue
            if re.search(r"(?i)art\.|ust\.|telefon|adres|data:", label):
                continue
            ...
            out.append(
                SalaryRow(
                    ...
                    institution_pk,
                    placowka,
                    norm_space(label[:140]),
                    "",
                    "",
                    None,
                    val,
                    page_no,
                    "explicit-annual-prose-salary",
                    "wysoka",
    ...
    if m:
        ...
        val = parse_money(token)
        label = norm_space(m.group(1)).strip(" .,:;-")
        if (
            val is not None
            and val > 0
            and re.search(r"(?i)lekarz|dyrektor|ordynator|kierownik", label)
        ):
            return [
                SalaryRow(
                    case_pk,
...
def parse_body_lekarz_number_amount(case_pk, institution_pk, placowka, page_no, page):
    ...
    if not re.search(
        r"(?is)(?:wynagrodze[nń]|zarobk).{0,260}2025|2025.{0,260}(?:wynagrodze[nń]|zarobk)",
        page,
    ):
        return []
    out = []
    ...
    for raw in lines:
        ...
        name = (
            f"Lekarz {m.group(1)}"
            if m.group(1)
            else "Lekarz " + norm_space(m.group(2)).strip(" .:-")
        )
    ...
    while i < len(lines) - 1:
        ...
        if not lm:
            ...
            continue
        j = i + 1
        if j < len(lines) and re.fullmatch(r"[-–—]", lines[j]):
            j += 1
        if j >= len(lines):
            break
        am = re.fullmatch(
    ...
    if len(out) >= 2:
        for i in range(len(lines) - 2):
            nm = lines[i]
            if not re.fullmatch(
                r"[A-ZĄĆĘŁŃÓŚŹŻ][A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż\-]+(?:\s+[A-ZĄĆĘŁŃÓŚŹŻ][A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż\-]+){1,3}",
                nm,
            ):
                continue
            j = i + 1
            ...
            if re.search(
                r"(?i)dyrektor|ordynator|kierownik|wynagrodz|lekarz|watchdog|sieć obywatelska",
                nm,
            ):
                continue
            token = am.group(1) + (("," + am.group(2)) if am.group(2) else "")
...
def parse_annual_amount_contract_lines(
    case_pk, institution_pk, placowka, page_no, page
):
    """Parse annual amount contract lines layouts into salary-row candidates."""
    if not (
        re.search(r"(?i)wynagrodzenia\s+lekarzy\s+za\s+rok\s+2025", page)
    ...
    for raw in page.splitlines():
        ...
        if (
            len(vals) == 1
            and ct
            and re.search(r"(?i)umow|kontrakt|cywilnopraw", raw)
            and not is_metadata_context(raw)
        ):
...
def parse_contract_practice_cost_list(case_pk, institution_pk, placowka, page_no, page):
    ...
    m = re.search(
        r"(?is)na\s+kontraktach\s+zatrudnionych\s+by[łl]o\s+(\w+|\d+)\s+lekarzy.{0,220}?koszt.{0,120}?2025.{0,180}?nast[eę]puj[aą]co\s*:",
        page,
    )
    if not m:
        return []
    ...
    block = re.sub(r"\n(?!\s*-)", " ", block)
    pat = re.compile(
        r"(?m)^\s*-\s*(.+?)\s*[—–-]\s*(\d{1,3}(?:[ .]\d{3})+(?:[,.]\d{2})\s*zł)\s*$"
    )
    for mm in pat.finditer(block):
        label = norm_space(mm.group(1))
        val = parse_money(mm.group(2))
        if val and val >= 1000:
            out.append(
                SalaryRow(
                    case_pk,
                    ...
                    "wysoka",
                    norm_space(mm.group(0)),
                )
            )
    return out if len(out) >= 2 else []


def parse_single_anonymized_annual_amount(
    case_pk, institution_pk, placowka, page_no, page
):
    """Parse single anonymized annual amount layouts into salary-row candidates."""
    if not (
        re.search(r"(?i)zanonimizowane\s+dane\s+osobowe", page)
        and re.search(r"(?is)wynagrodzenie\s+brutto.{0,60}(?:r[o0]k|tok)\s+2025", page)
    ):
        return []
    m = re.search(
        r"(?is)wynagrodzenie\s+brutto.{0,80}?(?:r[o0]k|tok)\s+2025.{0,80}?(\d{1,3}(?:[ .]\d{3})+(?:[,.]\d{2})\s*zł)",
        page,
    )
    if not m:
        return []
    val = parse_money(m.group(1))
    if not val or val < 1000:
        return []
    return [
        SalaryRow(
            case_pk,
            ...
            val,
            page_no,
            "single-anonymized-annual-amount",
            "wysoka",
            norm_space(m.group(0)),
        )
    ]
...
def parse_annual_named_colon_amount_list(
    case_pk, institution_pk, placowka, page_no, page
):
    """Parse annual named colon amount list layouts into salary-row candidates."""
    if not re.search(
        r"(?is)(?:zestawienie|wykaz|lista).{0,120}wynagrodze[nń].{0,120}(?:lekarz|2025)|wynagrodze[nń].{0,120}lekarz.{0,120}2025",
        page,
    ):
        return []
    out = []
    for line in page.splitlines():
        ...
        if re.search(
            r"(?i)(zestawienie|wynagrodze[nń]|telefon|nip|regon|krs|ul\.|sp\.j|sp\. z|rok\b)",
            name,
        ):
            continue
        token = m.group(2)
