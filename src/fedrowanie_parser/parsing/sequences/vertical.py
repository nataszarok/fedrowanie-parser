...
def parse_vertical_label_amount_pairs(
    case_pk: int,
    institution_pk: Optional[int],
    placowka: str,
    page_no: int,
    page: str,
    inherited_contract: str = "",
) -> list[SalaryRow]:
    ...
    for i in range(len(lines) - 1):
        ...
        if re.match("^\\d{1,4}[.)]?\\s+", label):
            continue
        if re.search(
            "(?i)razem|suma|wynagrodzen|strona|dyrektor|telefon|nip|regon|krs|brutto|netto|stawka",
            label,
        ):
            continue
        ...
        if len(vals) != 1:
            continue
        if not re.fullmatch(
            "\\s*(?:\\d{1,3}(?:[ .]\\d{3})+|\\d{1,7})(?:,\\d{1,2})?\\s*(?:zł)?\\s*",
            amount_line,
            re.I,
        ):
            continue
...
def parse_vertical_indexname_amount(
    case_pk: int,
    institution_pk: Optional[int],
    placowka: str,
    page_no: int,
    ...
    for i in range(len(lines) - 1):
        ...
        if len(vals) != 1:
            continue
        if not re.fullmatch(
            "\\s*\\d[\\d ]*(?:\\s*,\\s*\\d{1,2})?\\s*(?:zł)?\\s*", nxt, re.I
        ):
            continue
        val = vals[0]
        if val <= 0:
        ...
        rows.append(
            SalaryRow(
                ...
                val,
                page_no,
                "vertical-index-name-amount",
                "wysoka",
                f"{m.group(1)} {name} | {nxt}",
            )
        )
    if len(rows) < 5:
        return []
...
def parse_vertical_index_code_label_amount(
    ...
    while i < len(lines) - 2:
        ...
        if m and i + 2 < len(lines):
            ...
            if len(vals) == 1 and re.search(
                "(?i)lekarz|kontrakt|umow|kartotek|wynagrod|asystent|kierownik|stażysta|stazysta|rezydent",
                label,
            ):
                ...
                out.append(
                    SalaryRow(
                        ...
                        vals[0],
                        page_no,
                        "vertical-index-code-label-amount",
                        "wysoka",
                        " | ".join(lines[i : i + 3]),
                    )
                )
...
def parse_vertical_named_salary_table(case_pk, institution_pk, placowka, page_no, page):
    ...
    if not (
        re.search("(?i)imię i nazwisko|imie i nazwisko", page)
        and re.search("(?i)kwota wynagrodzenia.*2025", page)
    ):
        return []
    lines = [norm_space(x) for x in page.splitlines() if norm_space(x)]
    out = []
    ...
    while i + 2 < len(lines):
        if re.fullmatch("\\d{1,4}", lines[i]):
            ...
            vals = money_values(lines[i + 2])
            if (
                re.search("[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż]", name)
                and len(vals) == 1
                and (vals[0] > 0)
            ):
                out.append(
...
def parse_vertical_role_named_amount(case_pk, institution_pk, placowka, page_no, page):
    ...
    i = 0
    stop_re = re.compile(
        "(?i)^umowa\\b|^kontrakt\\b|^l\\.?p\\.?$|^wypłacone\\b|^nazwisko\\b"
    )
    while i < len(lines):
        low = lines[i].lower()
        ...
        if role and re.fullmatch("\\d{1,3}", lines[i]):
            ...
            while j < len(lines) and len(names) < 3:
                if stop_re.search(lines[j]) or lines[j].lower() in {
                    "ordynator",
                    "koordynator",
                }:
                    break
                vals = money_values(lines[j])
                if len(vals) == 1 and re.fullmatch(
                    "\\d[\\d ]*(?:,\\d{1,2})?", lines[j]
                ):
                    ...
                    if name:
                        out.append(
                            SalaryRow(
                                ...
                                vals[0],
                                page_no,
                                "vertical-role-named-amount",
                                "wysoka",
                                f"{role} | {idx} | {name} | {lines[j]}",
                            )
                        )
                    i = j + 1
                    found = True
...
def parse_two_section_vertical_salary(case_pk, institution_pk, placowka, page_no, page):
    ...
    if not (
        re.search("(?i)wynagrodzenie lekarzy zatrudnionych", page)
        and re.search("(?i)umow[ęe] o prac[ęe]", page)
        and re.search("(?i)umow[ęe] cywilnoprawn", page)
    ):
        return []
    lines = [norm_space(x) for x in page.splitlines() if norm_space(x)]
    out = []
    ...
    while i + 2 < len(lines):
        ...
        if m:
            ...
            if len(vals) == 1 and vals[0] > 0:
                ...
                continue
        idx = lines[i].rstrip(".")
        if re.fullmatch("\\d{1,4}", idx) and re.search(
            "(?i)dyżur|dyzur|praca dzienna", lines[i + 1]
        ):
            vals = money_values(lines[i + 2])
            if len(vals) == 1 and vals[0] > 0:
                out.append(
...
def parse_numbered_amount_only_series(case_pk, institution_pk, placowka, page_no, page):
    ...
    m = re.search(
        "(?is)wynagrodzenia.{0,160}(?:kształtowały|ksztaltowaly|następująco|nastepujaco)\\s*:\\s*(.+)",
        page,
    )
    if not m:
        return []
    tail = m.group(1)
    pat = re.compile(
        "(?<!\\d)(\\d{1,3})[.)]\\s*(\\d{1,3}(?:[ .]\\d{3})+|\\d{1,7})(?:,\\d{1,2})\\s*zł",
        re.I,
    )
    out = []
...
def parse_parallel_name_amount_lists(
    ...
    text = norm_space(doc)
    m = re.search(
        "(?is)imienna\\s+lista\\s+lekarzy.*?2025\\s*:?\\s*(.*?)łączne\\s+wynagrodzenie\\s+wypłacone\\s+w\\s*2025\\s*roku\\s*:?",
        text,
    )
    if not m:
        return []
    ...
    for mm in re.finditer(
        "(?<!\\d)(\\d{1,3}(?:[ .]\\d{3})+(?:,\\d{1,2})|\\d{1,6},\\d{1,2})\\s*zł",
        after,
        re.I,
    ):
    ...
    return [
        ...
        for i, (name, val) in enumerate(zip(names, amounts), 1)
    ]


def parse_parallel_doctor_amount_lists(
    case_pk, institution_pk, placowka, page_no, page
):
    """Parse parallel doctor amount lists layouts into salary-row candidates."""
    m1 = re.search(
        r"(?is)\bLekarze\s*:\s*(.+?)\bWynagrodzenia\s+lekarzy\s*:\s*(.+)", page
    )
    if not m1:
        return []
    ...
    if len(amount_parts) == len(names) + 1:
        ...
        candidates = [
            ...
            and "-" not in x
            and all(
                re.fullmatch(r"[A-ZĄĆĘŁŃÓŚŹŻ][A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż]+", w)
                for w in x.split()
            )
        ]
    ...
    for idx, (name, part) in enumerate(zip(names, amount_parts), 1):
        ...
        mm = re.fullmatch(
            r"(\d{1,3}(?:[ .]\d{3})+|\d{4,7}),(\d{2})\s*(?:zł)?[.,]?", part, re.I
        )
        if not mm:
    ...
    if unreadable:
        ...
        for r in out:
            r.komentarz = (
                (r.komentarz + "; " if r.komentarz else "")
                + "Lista równoległa; pominięto nieczytelną pozycję OCR: "
                + note
            )
    return out


def parse_anonymous_amount_only_series(
    case_pk, institution_pk, placowka, page_no, page
):
    ...
    for line in tail.splitlines():
        ...
        if re.search(r"(?i)^z poważaniem|^dyrektor\b|^prezes\b|^--$|^załącznik", raw):
            break
        mm = re.fullmatch(
            r"(\d{1,3}(?:[ .]\d{3})+|\d{4,7})(?:,\d{2})\s*(?:zł)?[.,]?", raw, re.I
        )
        if not mm:
