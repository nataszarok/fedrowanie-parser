...
from ..special_cases.multi_contract_columns import (
...
def parse_markdown_tables(
    ...
    while i < len(lines):
        ...
        semantic_lp, semantic_amount, semantic_person = semantic_annual_salary_columns(
            headers
        )
        table_context = "\n".join(lines[max(0, i - 12) : i + 2])
        exclude_table, _exclude_reason = table_should_be_excluded(
            headers, table_context
        )
        if exclude_table:
            i += 2
            while i < len(lines):
        ...
        if expand_stacked_contract_headers is not None and i < len(lines):
            ...
            effective, consumed = expand_stacked_contract_headers(
                headers_clean, first_cells
            )
            if consumed:
        ...
        while i < len(lines):
            ...
            if (
                is_metadata_or_date(raw_clean)
                or mentions_other_year(raw_clean)
                or is_summary_row(raw_clean)
            ):
                i += 1
                continue
            ...
            for col, (cell, h) in enumerate(zip(rcells, headers_clean)):
                ...
                financial_h = bool(
                    re.search(
                        "(?i)wynagrod|kwota|brutto|netto|zł|pln|<|>|500|mln|milion|umow|kontrakt|zlecen",
                        h,
                    )
                )
                ...
                if not financial_h and h:
                    continue
                for tok, val in vals:
                    if 1900 <= val <= 2100 and re.fullmatch(
                        "20\\d{2}(?:,0+)?", tok.strip().replace("zł", "").strip()
                    ):
                        continue
            ...
            paid_2025 = [
                ...
                if re.search(
                    "(?i)wynagrodzen(?:ie|ia).*wypłacone.*2025|wyplacone.*2025", h
                )
            ]
            if paid_2025:
            ...
            total_gross = [
                (col, h, tok, val)
                for col, h, tok, val in candidates
                if re.search("(?i)łącznie.*brutto|lacznie.*brutto", h)
            ]
            if total_gross:
                _, _, _, total_val = total_gross[-1]
                inferred = (
                    infer_contract_from_multi_columns(headers_clean, rcells)
            ...
            split_contracts = (
                split_multi_contract_amounts(headers_clean, rcells)
                if split_multi_contract_amounts is not None
                else []
            )
            if split_contracts:
            ...
            rows.append(
                SalaryRow(
                    ...
                    spec,
                    row_contract,
                    chosen if kind == "netto" else None,
                    chosen if kind == "brutto" else None,
                    page_no,
                    "markdown-table",
                    "wysoka",
                    raw_clean,
    ...
    no_header_4col = re.compile(
        "^\\|\\s*(?P<idx>\\d{1,4})[.)]?\\s*\\|\\s*(?P<label>[^|]+?)\\s*\\|\\s*(?P<contract>[^|]+?)\\s*\\|\\s*(?P<amount>\\d{1,3}(?:[ .]\\d{3})*(?:[,.]\\d{1,2})|\\d{4,7}(?:[,.]\\d{1,2}))\\s*(?:zł)?\\s*\\|?\\s*$",
        re.I,
    )
    ...
    for raw in lines:
        ...
        if raw.lstrip().startswith("|"):
            ...
            if (
                rc in existing_raw
                or is_summary_row(rc)
                or is_metadata_or_date(rc)
                or mentions_other_year(rc)
            ):
                continue
            ...
            continue
        if re.search(
            "(?i)\\b(?:ODDZIAŁ|PRACOWNIA|PORADNIA|NOCNA\\s+I\\s+ŚWIĄTECZNA\\s+OPIEKA)\\b",
            rc,
        ):
            current_section = rc
    existing_raw = {norm_space(r.raw_row) for r in rows}
    direct = re.compile(
        "^\\|\\s*(?P<idx>\\d{1,4})\\s*\\|\\s*(?P<amount>\\d{1,3}(?:[ .]\\d{3})*(?:[,.]\\d{1,2})?)\\s*(?:zł)?\\s*\\|\\s*(?P<label>[^|]*)\\|?\\s*$",
        re.I,
    )
    for raw in lines:
        rc = norm_space(raw)
    ...
    for i, raw in enumerate(lines):
        ...
        if not (
            "umow" in ctx
            and "prac" in ctx
            and ("kontrakt" in ctx or "cywilnopraw" in ctx)
        ):
            continue
                ...
                if mv0:
                    ...
                    rows.append(
                        SalaryRow(
                            ...
                            institution_pk,
                            placowka,
                            f"Lekarz UoP {left_no}",
                            "",
                            "umowa o pracę",
                            None,
                            mv0[0],
                            page_no,
                            "markdown-parallel-money-columns",
                            "wysoka",
                            rr,
                        )
                    )
    ...
    existing_raw = {norm_space(r.raw_row) for r in rows}
    empty_lp = re.compile(
        "^\\|\\s*\\|\\s*(?P<label>[^|]+?)\\s*\\|\\s*(?P<amount>\\d{1,3}(?:[ .]\\d{3})*(?:[,.]\\d{1,2})|\\d{4,7}(?:[,.]\\d{1,2}))\\s*(?:zł)?\\s*\\|?\\s*$",
        re.I,
    )
    for raw in lines:
