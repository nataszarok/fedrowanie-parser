...
PAGE_RE = re.compile(
    "-+\\s*początek strony\\s+(\\d+)\\s*-+\\s*(.*?)(?=-+\\s*koniec strony\\s+\\1\\s*-+|\\Z)",
    re.I | re.S,
)

MONEY_TOKEN = re.compile(
    "(?<![\\d.])(?:\\d{1,3}(?:[ \\u00a0]\\d{3})+|\\d{1,3}(?:\\.\\d{3})+|\\d{1,7})(?:[,.]\\d{1,2})?(?:\\s*zł)?(?![\\d])",
    re.I,
)
...
PHONEISH_RE = re.compile(
    "(?i)\\b(?:tel\\.?|fax|nip|regon|krs|sygn\\.?|znak|nr\\s+pisma|e-mail|email)\\b"
)

SUMMARY_RE = re.compile(
    "(?i)\\b(?:razem|suma|sumaryczn|ogółem|łącznie\\s+(?:wszyscy|wszystkich|wynagrodze[nń])|średni[ae]|średnia|mediana|minimum|maksimum|wartość\\s+łączna|podsumowanie|ogólna\\s+kwota)\\b"
)

PERSON_TOTAL_RE = re.compile(
    "(?i)\\b(?:łączne|roczne|całkowite)\\s+wynagrodzenie\\s+(?:lekarza|lp\\.?\\s*\\d+|nr\\s*\\d+)"
)
...
CONTRACT_PATTERNS = [
    (
        re.compile("(?i)umow[ayę]\\s+o\\s+prac[ęe]|etat|stosunek\\s+pracy"),
        "umowa o pracę",
    ),
    (re.compile("(?i)umow[ayę]\\s+zlecen|zlecenie"), "umowa zlecenia"),
    (
        re.compile("(?i)kontrakt|cywilno[- ]?prawn|działalnoś[ćc]\\s+gospodar"),
        "kontrakt/cywilnoprawna",
    ),
    (re.compile("(?i)umow[ayę]\\s+o\\s+dzieło|dzieło"), "umowa o dzieło"),
]
...
SPECIALIZATION_WORDS = re.compile(
    "(?i)\\b(?:chirurg|ginekolog|położ|internist|pediatr|okulist|neurolog|psychiatr|kardiolog|anestez|ortoped|urolog|radiolog|onkolog|laryng|otolaryng|dermatolog|nefrolog|hematolog|endokrynolog|gastrolog|pulmonolog|reumatolog|medycyn|rezydent|asystent|ordynator|kierownik|lekarz)\\b"
)
...
THREAD_DATE_RE = re.compile(
    "^\\d{1,2}\\s+(?:stycznia|lutego|marca|kwietnia|maja|czerwca|lipca|sierpnia|września|października|listopada|grudnia)\\s+20\\d{2}\\s+\\d{1,2}:\\d{2}$",
    re.I,
)

AUTO_REPLY_RE = re.compile(
    "(?i)Kategoryzacja AI została pominięta dla listu z automatyczną odpowiedzią|To jest potwierdzenie dostarczenia|automatyczn[aey]\\s+odpowied|potwierdzenie otrzymania"
)

REQUESTER_SENDER_RE = re.compile(
    "(?i)^(?:SOWP|Sieć Obywatelska Watchdog Polska|Siec Obywatelska Watchdog Polska)$"
)

QUOTE_CUT_PATTERNS = [
    re.compile("(?m)^W dniu .+ napisał\\(a\\):\\s*$", re.I),
    re.compile("(?m)^-{2,}\\s*Original Message\\s*-{2,}\\s*$", re.I),
    re.compile("(?m)^From:\\s+.+$", re.I),
    re.compile("(?m)^Od:\\s+.+$", re.I),
]

PARSER_RANK = {
    "ocr-broken-numbered-table": 11,
    "parallel-name-amount-lists": 10,
    "vertical-idx-code-gross-net": 9,
    "parallel-index-amount": 9,
    "anon-lekarz-inline-list": 9,
    "indexed-single-salary-md": 8,
    "indexed-total-gross-continuation": 9,
    "salary-bracket-index": 10,
    "embedded-numbered-salary-list": 10,
    "numbered-amount-only-series": 10,
    "specialty-amount-lines": 10,
    "vertical-role-named-amount": 10,
    "vertical-named-salary-table": 10,
    "two-section-vertical": 10,
    "explicit-annual-prose-salary": 10,
    "vertical-index-code-label-amount": 9,
    "numbered-named-inline": 9,
    "body-lekarz-number-amount": 10,
    "body-lp-amount": 10,
    "parallel-doctor-amount-lists": 10,
    "anonymous-amount-only-series": 10,
    "annual-amount-contract-lines": 10,
    "contract-practice-cost-list": 10,
    "forma-name-amount-ocr": 9,
    "single-anonymized-annual-amount": 10,
    "lekarz-inline-specialty": 9,
    "markdown-section-state": 6,
    "markdown-net-gross": 5,
    "markdown-multi-contract": 5,
    "markdown-table": 4,
    "markdown-paid-2025": 4,
    "markdown-empty-lp": 4,
    "markdown-continuation": 4,
    "markdown-3col-continuation": 4,
    "vertical-label-amount": 5,
    "vertical-index-name-amount": 6,
    "ocr-contract-amount-list": 5,
    "plain-inline": 3,
    "plain-index-amount": 2,
    "plain-vertical": 1,
}

STRUCTURED_METADATA_PARSERS = {
    "contract-practice-cost-list",
    "indexed-single-salary-md",
    "indexed-total-gross-continuation",
    "salary-bracket-index",
    "numbered-named-inline",
    "anon-lekarz-inline-list",
}
...
MULTI_CONTRACT_MONEY_RE = re.compile(
    r"-?\d{1,3}(?:[ .]\d{3})*(?:[,.]\d{2})|-?\d{4,9}(?:[,.]\d{2})"
)
...
ATTACHMENT_CONTRACT_PATTERNS = [
    ...
    (
        ...
        re.compile(r"(?i)dzia[łl]alno[śs][ćc][\s._-]+gospodarcz\w*"),
    ),
]

# enrichment/doctor_initials.py
DOCTOR_INITIALS_MONEY_RE = re.compile(
    r"^\s*-?\d{1,3}(?:[ .]\d{3})*(?:[,.]\d{2})\s*(?:z[łl]|PLN)?\s*$", re.I
)
...
# enrichment/unit_column.py
UNIT_COLUMN_MONEY_RE = re.compile(
    r"-?\d{1,3}(?:[ .]\d{3})*(?:[,.]\d{2})|-?\d{4,9}(?:[,.]\d{2})"
)
...
# enrichment/organizational_unit.py
ORGANIZATIONAL_CONTACT_RE = re.compile(
    r"(?i)\b(?:tel\.?|fax|e-?mail|www\.|ul\.|telefon)\b"
)
...
# enrichment/organizational_unit.py
ORGANIZATIONAL_ROLE_RE = re.compile(
    r"(?i)^(?:koordynator|zast[ęe]pca koordynatora|z-ca koordynatora|"
    r"ordynator|zast[ęe]pca ordynatora|kierownik|p\.?o\.?.*)$"
)

# enrichment/person_name.py
PERSON_NAME_HEADER_RE = re.compile(
    r"(?i)^(?:nazwisko\s+i\s+imi[ęe]|nazwisko\s+imi[ęe]|imi[ęe]\s+i\s+nazwisko|imi[ęe]\s+nazwisko|nazwisko(?:\s+lekarza)?|dane\s+lekarza)$"
)

# enrichment/person_name.py
PERSON_NAME_MONEY_RE = re.compile(
    r"-?\d{1,3}(?:[ .]\d{3})*(?:[,.]\d{2})|-?\d{4,9}(?:[,.]\d{2})"
)

# enrichment/person_name.py
PERSON_NAME_BAD_RE = re.compile(
    r"(?i)\b(?:lekarz|specjalista|specjalizacja|asystent|rezydent|ordynator|koordynator|kierownik|zast[ęe]pca|oddzia[łl]|klinika|poradnia|pracownia|zak[łl]ad|o[śs]rodek|chirurg|ginekolog|pediatr|psychiatr|radiolog|kardiolog|neurolog|nefrolog|urolog|onkolog|anestez|medycyn|umowa|kontrakt|etat|wynagrodzenie|kwota|brutto|netto)\b"
)
...
SPECIALIZATION_PATTERNS = [
    (
        ...
        re.compile(r"(?i)\bradiologia\s+i\s+diagnostyka\s+obrazowa\b"),
    ),
    (
        "radiologia",
        re.compile(r"(?i)\bradiolog(?:ia|ii|icz\w*|a|iem)?\b|\bradiodiagnost\w*\b"),
    ),
    (
        "psychiatria dzieci i młodzieży",
        re.compile(r"(?i)\bpsychiatri\w+\s+dzieci\s+i\s+m[łl]odzie[żz]y\b"),
    ),
    ("psychiatria", re.compile(r"(?i)\bpsychiatr\w*\b")),
    (
        "anestezjologia i intensywna terapia",
        re.compile(r"(?i)\banestezjolog\w*(?:\s+i\s+intensywn\w+\s+terapi\w*)?\b"),
    ),
    ("neurochirurgia", re.compile(r"(?i)\bneurochirurg\w*\b")),
    ("hematologia", re.compile(r"(?i)\bhematolog\w*\b")),
    (
        "chirurgia klatki piersiowej",
        re.compile(r"(?i)\bchirurg\w+\s+klatki\s+piersiow\w*\b"),
    ),
    ...
    (
        ...
        re.compile(
            r"(?i)\b(?:ginekolog\w*.{0,20}po[łl]o[żz]nictw\w*|po[łl]o[żz]nictw\w*.{0,20}ginekolog\w*)\b"
        ),
    ),
    ("ginekologia", re.compile(r"(?i)\bginekolog\w*\b")),
    (
        "choroby wewnętrzne",
        re.compile(r"(?i)\bchor[oó]b\w+\s+wewn[ęe]trzn\w*|\binternist\w*\b"),
    ),
    ...
    (
        ...
        re.compile(
            r"(?i)\bortoped\w*.{0,30}traumatolog\w*|\btraumatolog\w*.{0,30}ortoped\w*"
        ),
    ),
...
DOCTOR_STATUS_PATTERNS = [
    (
        re.compile(r"(?i)\blekarz\s+w\s+trakcie\s+specjalizacji\b"),
    ...
    (
        re.compile(
            r"(?i)\bz(?:ast[ęe]pca|-ca)\s+(?:ordynatora|koordynatora|kierownika)\b"
        ),
        "zastępca kierownika/koordynatora",
    ),
]
