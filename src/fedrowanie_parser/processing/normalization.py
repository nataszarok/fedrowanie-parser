...
def is_metadata_or_date(line: str) -> bool:
    ...
    if PHONEISH_RE.search(l) and (not SPECIALIZATION_WORDS.search(l)):
        return True
    if re.search(
        "(?i)\\b(?:godzina|data\\s+odebrania|wygenerowano|załącznik\\s+nr|strona\\s+\\d+)\\b",
        l,
    ):
        return True
    return False
...
def detect_contract(text: str, fallback: str = "") -> str:
    ...
    t = text or ""
    has_work = bool(
        re.search("(?i)umow[ayę]\\s+o\\s+prac[ęe]|etat|stosunek\\s+pracy", t)
    )
    has_order = bool(re.search("(?i)umow[ayę]\\s+zlecen|zlecenie", t))
    has_contract = bool(
        re.search("(?i)kontrakt|cywilno[- ]?prawn|działalnoś[ćc]\\s+gospodar", t)
    )
...
def header_has_money_context(headers: list[str]) -> bool:
    ...
    return bool(
        re.search(
            "(?i)wynagrod|kwota|brutto|netto|2025|zł|pln|kontrakt|zlecen|umowa", h
        )
    )


def infer_name_and_spec(
    cells: list[str], headers: list[str], idx_col: Optional[int]
) -> tuple[str, str]:
    ...
    if not name and text_candidates:
        for c in text_candidates:
            if re.fullmatch("(?:[A-ZĄĆĘŁŃÓŚŹŻ]\\.?\\s*){2,4}", c) or re.search(
                "(?i)\\blekarz\\b", c
            ):
                name = c
                break
...
def parse_idx(
    cells: list[str], headers: list[str]
...
def table_should_be_excluded(headers: list[str], context: str) -> tuple[bool, str]:
    ...
    if re.search(r"(?i)liczba\s+lekarzy|liczba\s+osób|liczba\s+osob", h) and re.search(
    ...
    roman_month_cols = sum(
        (
            1
            for x in headers
            if re.fullmatch(
                "(?i)(?:i|ii|iii|iv|v|vi|vii|viii|ix|x|xi|xii)", clean_header(x)
            )
        )
    )
    has_annual_total = bool(
        re.search(
            "(?i)(?:łącznie|lacznie|suma|wynagrodzenie).*2025|2025.*(?:łącznie|lacznie|suma)",
            h,
        )
    )
    ...
    group_dimension = bool(
        re.search(
            "(?i)\\b(?:oddział|oddzial|poradnia|komórka|komorka|jednostka|klinika|zakład|zaklad)\\b",
            h + " " + c,
        )
    )
    if has_min and has_max and group_dimension:
        return (True, "group_min_max_statistics")
    explicit_other_period = re.search(
        "(?i)\\b(?:dane\\s+za|za\\s+miesiąc|za\\s+miesiac|maj|stycze[nń]|luty|marzec|kwiecie[nń]|czerwiec|lipiec|sierpie[nń]|wrzesie[nń]|październik|pazdziernik|listopad|grudzie[nń]).{0,40}\\b(20\\d{2})\\b",
        c,
    )
...
def semantic_annual_salary_columns(
    headers: list[str],
    ...
    amount = next(
        (
            ...
            if re.search(
                "(?i)wynagrodzenie.*brutto.*2025|brutto.*2025.*wynagrodzenie", h
            )
        ),
        None,
    )
    person = next(
        (
            i
            for i, h in enumerate(hs)
            if re.search("(?i)stanowisko|lekarz|osoba|kod anonimowy|nazwa", h)
            and i != amount
        ),
        None,
    )
