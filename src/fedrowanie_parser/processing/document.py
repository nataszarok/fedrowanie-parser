...
def split_pages(text: str) -> list[tuple[int, str]]:
    ...
    marker_blocks = list(
        re.finditer(
            "-+\\s*początek strony\\s+\\d+\\s*-+.*?-+\\s*koniec strony\\s+\\d+\\s*-+",
            src,
            re.I | re.S,
        )
    )
    ...
    for chunk in spans:
        ...
        salary_sem = bool(
            re.search(
                "(?i)wynagrod|gospodarcza|cywilno.?prawna|kontrakt|umowa o pracę|specjalista|lekarz",
                chunk,
            )
        )
...
def page_is_2025_relevant(page: str, doc: str) -> bool:
    ...
    if years and YEAR not in years:
        ...
        correspondence = re.search(
            "(?i)wniosek|odpowiedzi na wniosek|data odebrania|temat:|koszalin, dnia|szczecin, dnia",
            page,
        )
        doc_years = {int(x) for x in YEAR_OTHER_RE.findall(doc)}
        salary_rich = len(money_cells(page)) >= 10 and bool(
            re.search(
                "(?i)wynagrod|gospodarcza|cywilno.?prawna|qwilno.?prawna|kontrakt|umowa o pracę|lekarz|specjalista",
                page,
            )
        )
        explicit_wrong_period = bool(
            re.search(
                "(?i)(?:stycze[nń]|luty|marzec|kwiecie[nń]|maj|czerwiec|lipiec|sierpie[nń]|wrzesie[nń]|październik|pazdziernik|listopad|grudzie[nń]).{0,30}\\b(?:2024|2026|2027)\\b",
                page,
            )
        )
        if not (
            tableish
            and YEAR in doc_years
            and (not correspondence)
            or (salary_rich and YEAR in doc_years and (not explicit_wrong_period))
        ):
            return False
    return True
...
def deduplicate(rows: Iterable[SalaryRow]) -> list[SalaryRow]:
    ...
    rows = list(rows)
    markdown_amounts = {
        (r.case_pk, round(r.brutto if r.brutto is not None else r.netto, 2))
        for r in rows
        if r.parser.startswith("markdown")
        and (r.brutto is not None or r.netto is not None)
    }
    seen = set()
    seen_numbered = set()
    out = []
    for r in rows:
        ...
        if (
            r.parser == "plain-split"
            and ("| --- |" in raw or "|---|" in raw)
            and ((r.case_pk, round(val, 2)) in markdown_amounts)
        ):
            continue
        exact = (
            r.case_pk,
            r.strona,
            norm_space(r.nazwa).lower(),
            norm_space(r.specjalizacja).lower(),
            norm_space(r.typ_umowy).lower(),
            r.netto,
            r.brutto,
            raw,
        )
...
def classify_nonannual_response(doc: str) -> tuple[bool, str]:
    ...
    invoice_code_count = len(
        re.findall("\\b[A-ZŻŹĆĄŚĘŁÓŃ]{1,8}/2025/\\d{1,2}/\\d+\\b", doc)
    )
    ...
    tariff_core = re.search(
        "(?i)współczynnik\\s+pracy|wspolczynnik\\s+pracy", doc
    ) and re.search("(?i)wynagrodzenie\\s+zasadnicze", doc)
    rate_core = re.search(
        "(?i)stawki\\s+lekarzy|rozliczenie\\s+następuje\\s+za\\s+godzin|rozliczenie\\s+nastepuje\\s+za\\s+godzin|punkty\\s+za\\s+wykonane\\s+procedury|procedury.*stawki",
        doc,
    )
    ...
    for line in doc.splitlines():
        ...
        romans = len(
            re.findall(
                "\\|\\s*(?:I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII)\\s*(?=\\|)",
                line,
                re.I,
            )
        )
    ...
    header_lines = [
        line.lower() for line in doc.splitlines() if line.strip().startswith("|")
    ]
    monthly_header = any(
        (
            ("miesiąc" in line or "miesiac" in line)
            and sum(
                (
                    k in line
                    for k in [
                        "najwyższe miesięczne",
                        "najwyzsze miesieczne",
                        "średnie miesięczne",
                        "srednie miesieczne",
                        "najniższe miesięczne",
                        "najnizsze miesieczne",
                        "mediana",
                    ]
                )
            )
    ...
    if monthly_header:
        ...
        for line in doc.splitlines():
            if re.search(MONTH_NAME_RE, line, re.I):
                continue
            if re.search(
                "\\b[A-ZĄĆĘŁŃÓŚŹŻ][a-ząćęłńóśźż-]+\\s+[A-ZĄĆĘŁŃÓŚŹŻ][a-ząćęłńóśźż-]+\\b",
                line,
            ) and re.search("\\d{1,3}(?:[ .]\\d{3})+(?:,\\d{1,2})?\\s*zł", line, re.I):
                annual_named += 1
        if annual_named < 2:
            return (True, "monthly_group_statistics_not_individual_annual_list")
    if re.search(
        "(?is)\\|\\s*Oddziały\\s*\\|.*?Minimalny\\s+przychód\\s+za\\s+2025.*?Maksymalny\\s+przychód\\s+za\\s+2025",
        doc,
    ) and re.search(
        "(?i)uniemożliwia\\s+porównanie.*poszczególnych\\s+lekarzy|uniemozliwia\\s+porownanie.*poszczegolnych\\s+lekarzy",
        doc,
    ):
        return (True, "group_min_max_not_individual_salaries")
    wrong_period = re.search(
        f"(?is)Dane\\s+za\\s*\\|?\\s*{MONTH_NAME_RE}\\s+(20\\d{{2}})\\s*r?\\.?", doc
    )
    if wrong_period and wrong_period.group(1) != "2025":
        if re.search(
            "(?i)wynagrodzenie\\s+(?:brutto|netto)\\s+za\\s+1\\s+miesiąc|średnia\\s+wszystkich\\s+lekarzy|mediana\\s+wszystkich\\s+lekarzy",
            doc,
        ):
    ...
    if not headers:
        return (
            "",
            {
                ...
                "substantive_recipient_messages": 0,
            },
        )
    recipient_parts = []
    recipient_messages = 0
    ...
    for j, (i, subject, sender, date) in enumerate(headers):
        ...
        if cut_at is not None:
            ...
            attach_positions = [
                p
                for marker in ("Załączniki", "Załącznik", "początek strony")
                for p in [after_quote.find(marker)]
                if p >= 0
            ]
            if attach_positions:
        ...
        recipient_parts.append(
            f"### WIADOMOŚĆ ODBIORCY\nTemat: {subject}\nData: {date}\n{clean}"
        )
    return (
        "\n\n".join(recipient_parts),
        {
            "thread_detected": True,
            "all_messages": len(headers),
            "recipient_messages": recipient_messages,
            "substantive_recipient_messages": substantive,
        },
    )
...
def correspondence_status(doc: str, rows: list[SalaryRow]) -> tuple[str, str]:
    ...
    refusal = bool(
        re.search(
            "(?is)(?:odmaw\\w*|nie\\s+udostępni\\w*|nie\\s+udzieli\\w*|nie\\s+jest\\s+informacją\\s+publiczną|podlega\\s+ograniczeniu).{0,300}(?:wynagrodze[nń]|imion|nazwisk|danych\\s+osobowych|prywatnoś|prywatnos)",
            doc,
        )
    )
    anonymized = bool(
        re.search(
            "(?i)(?:anonimiz|zanonimiz|bez\\s+podawania\\s+imion|bez\\s+imion\\s+i\\s+nazwisk|zestawienie\\s+nie\\s+zawiera\\s+imion|kod\\s+anonimowy|lekarz\\s+\\d+)",
            doc,
        )
    )
    distribution = bool(
        re.search("(?is)liczba\\s+lekarzy.{0,180}wynagrodzen.{0,140}przedzia", doc)
    )
    date_count = len(re.findall("\\b2025-\\d{2}-\\d{2}\\b", doc))
    invoice_code_count = len(
        re.findall("\\b[A-ZŻŹĆĄŚĘŁÓŃ]{1,8}/2025/\\d{1,2}/\\d+\\b", doc)
    ...
    tariffs = bool(
        re.search("(?i)współczynnik\\s+pracy|wspolczynnik\\s+pracy", doc)
        and re.search("(?i)wynagrodzenie\\s+zasadnicze", doc)
        and re.search(
            "(?i)stawki\\s+lekarzy|rozliczenie\\s+następuje\\s+za\\s+godzin|rozliczenie\\s+nastepuje\\s+za\\s+godzin|punkty\\s+za\\s+wykonane\\s+procedury|procedury.*stawki",
            doc,
    ...
    for line in doc.splitlines():
        ...
        if romans >= 4 and (not re.search("(?i)łącznie|lacznie|suma|2025", line)):
            ...
            break
    monthly_group_stats = any(
        (
            ("miesiąc" in line.lower() or "miesiac" in line.lower())
            and sum(
                (
                    k in line.lower()
                    for k in [
    ...
    wrong_period = bool(
        re.search(
            f"(?is)Dane\\s+za\\s*\\|?\\s*{MONTH_NAME_RE}\\s+(?:2024|2026|2027)\\s*r?\\.?",
            doc,
        )
        and re.search(
            "(?i)wynagrodzenie\\s+(?:brutto|netto)\\s+za\\s+1\\s+miesiąc|średnia\\s+wszystkich\\s+lekarzy|mediana\\s+wszystkich\\s+lekarzy",
    ...
    annual_rows = [
        r
        for r in rows
        if (r.brutto is not None or r.netto is not None)
        and ((r.brutto or 0) > 0 or (r.netto or 0) > 0)
    ]
    if invoices:
        return ("SUBSTANTIVE_NONCOMPARABLE_DATA", "accounting_invoice_register")
    if tariffs:
        return (
            "SUBSTANTIVE_NONCOMPARABLE_DATA",
            "tariffs_coefficients_or_procedure_rates",
        )
    if periodic_matrix:
        return (
            "SUBSTANTIVE_NONCOMPARABLE_DATA",
            "periodic_month_matrix_without_annual_total",
        )
    if group_minmax:
    ...
    if distribution:
        genuine_rows = [
            ...
            if not re.search(
                "(?i)liczba\\s+lekarzy|przedzia[łl]|^\\s*\\d+(?:\\s*\\|\\s*\\d+){1,3}\\s*$",
                norm_space(r.raw_row),
            )
            and (
                re.search(
                    "(?i)kod\\s+anonimowy|lekarz\\s+\\d+|praktyka\\s+lekarska",
                    r.raw_row,
                )
                or (
                    r.nazwa
                    and (not re.fullmatch("(?i)lekarz\\s*\\d*", norm_space(r.nazwa)))
                )
            )
        ]
        if not genuine_rows:
    ...
    if monthly_group_stats:
        ...
        if explicit_annual_named < 2:
            return (
                "SUBSTANTIVE_AGGREGATE_ONLY",
                "monthly_group_statistics_not_individual_annual_list",
            )
    if annual_rows:
        if refusal:
            return (
                "INDIVIDUAL_ANNUAL_2025_AFTER_OR_WITH_REFUSAL",
                "anonymized" if anonymized else "data_after_refusal",
            )
        return ("INDIVIDUAL_ANNUAL_2025", "individual_or_anonymous_values")
    aggregate = bool(
        re.search(
            "(?is)(?:sumaryczna|łączna|laczna)\\s+(?:wartość|wartosc)?\\s*wynagrodze[nń].{0,160}2025|(?:średnia|srednia|mediana|minimalny|maksymalny|najwyższe|najwyzsze|najniższe|najnizsze).{0,180}(?:wynagrodzen|przychod)",
            doc,
        )
    )
    if aggregate:
...
def topn_comment(doc: str) -> str:
    ...
    if nums:
        ...
        if len(uniq) == 1:
            n = uniq[0]
            occurrences = len(
                re.findall(rf"(?i)\b{n}\s+lekarzy\s+z\s+najwy[żz]sz", text)
            )
            if occurrences >= 2:
...
def is_metadata_context(text: str) -> bool:
    ...
    return bool(
        re.search(
            r"(?i)kapita[łl]\s+zak[łl]adowy|\bKRS\b|\bREGON\b|\bNIP\b|kapita[łl]\s+sp[oó][łl]ki|s[ąa]d\s+rejonowy|rejestr\w*\s+s[ąa]dow",
            text or "",
        )
    )
...
def filter_registry_capital_false_rows(rows, doc: str):
    ...
    for r in rows:
        ...
        for i, line in enumerate(lines):
            ...
            if is_metadata_context(ctx):
                bad = True
                break
        if not bad:
            # Also catch one-line OCR: "Kapitał zakładowy: 65 559 000,00 zł".
            pat = rf"(?is)(?:kapita[łl]\s+zak[łl]adowy|\bKRS\b|\bREGON\b|\bNIP\b).{{0,120}}\b{re.escape(idx)}\s+{int(val):,}".replace(
                ",", r"[ .]?"
            )
            if re.search(pat, doc or ""):
                bad = True
        if not bad:
            out.append(r)
    return out
...
def aggregate_count_total_guard(doc: str) -> bool:
    ...
    count = bool(
        re.search(
            r"(?i)(?:ilo[śs][ćc]|liczba)\s+(?:zatrudnionych\s+)?lekarzy.{0,80}?\b\d{1,5}\s*(?:os[oó]b)?",
            t,
        )
    )
    total = bool(
        re.search(
            r"(?i)(?:[łl][ąa]czna|sumaryczna|suma)\s+(?:kwota\s+)?(?:wyp[łl]aconych\s+)?wynagrodze[nń].{0,100}?\d{1,3}(?:[ .]\d{3})+",
            t,
        )
    )
...
def is_correction_message(text: str) -> bool:
    ...
    t_wo_rodo = re.sub(r"(?is)prawo.{0,80}sprostowan.{0,120}(?:danych|osobow)", " ", t)
    ...
    explicit = bool(
        ...
        or re.search(
            r"(?is)b[łl][ąa]d\w*.{0,180}(?:ksi[eę]gow|kwot|wykaz|zestaw|wynagrod)",
            t_wo_rodo,
        )
        or re.search(
            r"(?is)omy[łl]k\w*.{0,180}(?:kwot|wykaz|zestaw|wynagrod)", t_wo_rodo
        )
    )
    return explicit
...
def prefer_latest_correction(doc: str) -> str:
    ...
    if corr:
        ...
        ms = list(
            re.finditer(
                r"(?i)\b(?:korekt\w*|skorygow\w*|poprawion\w*|sprostowan\w*)\b", part
            )
        )
        if ms:
            pos = ms[-1].start()
            anchors = [
                m.start()
                for m in re.finditer(r"(?m)^# \*\*Tabela\s+1\*\*\s*$", part[:pos])
            ]
            if anchors:
...
def is_wrong_month(text: str) -> bool:
    ...
    return bool(
        re.search(
            "(?i)(?:stycze[nń]|luty|marzec|kwiecie[nń]|maj|czerwiec|lipiec|sierpie[nń]|wrzesie[nń]|październik|pazdziernik|listopad|grudzie[nń]).{0,40}\\b(?:2024|2026|2027)\\b",
            text,
        )
    )
...
def recipient_document(full_doc: str) -> tuple[str, dict]:
    ...
    if len(money_cells(doc)) >= 10 or not re.search(
        "(?m)^A\\) email jest odpowiedzią", full_doc
    ):
    ...
    if candidates:
        ...
        meta.update(
            thread_detected=True,
            substantive_recipient_messages=1,
            fallback_a_message=True,
        )
...
def page_is_group_aggregate_salary_table(page: str) -> bool:
    ...
    for i, line in enumerate(lines[:-1]):
        ...
        if re.search(
            r"(?i)liczba\s+lekarzy|liczba\s+osób|liczba\s+osob", h
        ) and re.search(
            r"(?i)łączna\s+kwota\s+wynagrodze[nń]|laczna\s+kwota\s+wynagrodze[nń]|suma\s+wynagrodze[nń]",
            h,
        ):
            return True
    return False
