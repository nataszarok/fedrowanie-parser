...
def _page_rows(
    ...
    if vals and max(vals) < 1000 and not re.search(r"[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż]", page):
        return []
    ...
    md = parse_markdown_tables(
        *args, inherited_contract, inherited_kind, inherited_spec
    )
    ...
    vertical_named = [
        r
        for r in vertical_named
        if not re.fullmatch("[\\d\\s.,]+\\s*zł", r.nazwa or "", re.I)
    ]
    if existing_vertical:
        ...
        plain = [
            r for r in plain if r.parser not in ("plain-index-amount", "plain-inline")
        ]
    if embedded_list:
        named_inline = []
    strong = (
        ...
        or parallel_lists
        or anon_amounts
        or annual_contract_lines
        or contract_practice_costs
        or forma_rows
        or single_anon_annual
        or named_colon_annual
    )
    ...
    if strong:
        plain = [
            r
            for r in plain
            if r.parser not in ("plain-inline", "plain-index-amount", "plain-vertical")
        ]
    groups = (
        md,
        md_cont,
...
def _technical_filter(rows):
    ...
    for r in deduplicate(rows):
        if suspicious_amount(r) or is_summary_row(r.raw_row):
            continue
        if (
            is_metadata_or_date(r.raw_row)
            and r.parser not in STRUCTURED_METADATA_PARSERS
        ):
            continue
        if re.search(
            "(?i)<\\s*500[ .]?000|>\\s*500[ .]?000|>\\s*1[ .]?000[ .]?000", r.raw_row
        ):
            continue
        if (
            r.parser == "markdown-table"
            and (r.brutto or r.netto or 0) < 1000
            and re.search("(?i)okres:|od 01 do 12", r.raw_row)
        ):
            continue
        out.append(r)
    return out
...
def _case_rows(case_pk, institution_pk, placowka, doc):
    ...
    if parallel:
        return filter_registry_capital_false_rows(
            _rank_dedup(_technical_filter(parallel)), doc
        )
    ...
    def amounts(rs):
        return [
            round((r.brutto if r.brutto is not None else r.netto or 0), 2) for r in rs
        ]
        ...
        if mcmap:
            ...
            for rk, expected in mcmap.items():
                ...
                if total_match is not None:
                    total_match.typ_umowy = " / ".join(
                        dict.fromkeys(ct for ct, _ in expected)
                    )
                    continue
                    ...
                    if existing:
                        ...
                        additions.append(
                            SalaryRow(
                                ...
                                amount,
                                proto.strona,
                                "markdown-multi-contract-reconcile",
                                "wysoka",
                                proto.raw_row,
                                (proto.komentarz or "")
                                + "; odzyskano brakującą niepustą kolumnę typu umowy",
                            )
                        )
    ...
    if parse_section_salary_list is not None:
        ...
        if sec_items:
            ...
            if len(sec_items) >= len(final_rows) and overlap >= max(
                10, int(0.85 * len(final_rows))
            ):
    ...
    if document_unit_row_map is not None or section_unit_map is not None:
        ...
        idxmap = (
            document_unit_index_amount_map(doc)
            if document_unit_index_amount_map is not None
            else {}
        )
        ...
        for r in final_rows:
            ...
            if m_idx and rv is not None:
                idx_exact = idxmap.get(
                    (int(m_idx.group(1)), int(round(float(rv) * 100)))
                )
    ...
    if document_person_name_maps is not None or infer_person_name is not None:
        raw_name_map, idx_name_map = (
            document_person_name_maps(doc)
            if document_person_name_maps is not None
            else ({}, {})
        )
        for r in final_rows:
            key = " | ".join(
                norm_space(x)
                for x in str(r.raw_row or "").strip().strip("|").split("|")
            )
            ...
            if person:
                ...
                if (r.specjalizacja or "").strip() and norm_space(
                    r.specjalizacja
                ).casefold() == norm_space(person).casefold():
                    r.specjalizacja = ""
